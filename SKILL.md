...
转录质量策略：`transcribe.py` 会同时保存两份文本，默认 `<视频名>.txt` 为可读稿，用于入库和 Dashboard；`<视频名>.raw.txt` 为原始 Whisper 分段稿，用于回查原始识别结果。可读稿会按中文口播语法做保守断句：优先识别“第一招/第二招/首先/然后/但是/所以/比如/正确写法/错误写法”等句群边界，再补基础句末标点；找不到合适中文断点时，不要硬切词，宁可保留稍长句子。长视频教程入库后，应对转录做一遍 AI 语义校对：合并被切断的词句，修正明显术语识别错误，补自然标点，但不新增原视频没有的观点。长视频不要默认把逐字完整版全文塞进笔记正文；正文优先展示 AI 语义整理稿，并在该区块顶部挂上完整纯文本字幕和原始 VTT/原始分段稿的本地链接。只有用户明确要求“全文展示/逐字全文放进笔记”时，才把完整版直接写入笔记。AI 语义校对稿或人工整理稿导出时必须原样保留，不要再走自动断句；只有明确需要把原始单行稿转成初稿时，才给 `export_to_obsidian.py` 加 `--format-transcript`。自动标点适合初稿；如果用户要求发布级逐字稿、法律/合同级准确转录，必须再做 AI 校对或人工校对。
...
- “按主题投喂”。
- 单篇笔记“复制给AI”。
- 单篇长视频“全文投喂AI”：复制完整纯文本字幕/转录、资料路径、摘要和 AI快速参考；详情页可一键打开完整字幕原文件。
- 本地视频空间提醒和“删除本地视频”。
- 本地文件入库命令生成入口。
- 回收站查看与恢复。
