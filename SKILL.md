...
转录质量策略：`transcribe.py` 会同时保存两份文本，默认 `<视频名>.txt` 为可读稿，用于入库和 Dashboard；`<视频名>.raw.txt` 为原始 Whisper 分段稿，用于回查原始识别结果。可读稿会按中文口播语法做保守断句：优先识别“第一招/第二招/首先/然后/但是/所以/比如/正确写法/错误写法”等句群边界，再限制单行长度并补基础句末标点。自动标点适合知识库阅读和 AI 投喂；如果用户要求发布级逐字稿、法律/合同级准确转录，必须在入库后再做 AI 校对或人工校对。
