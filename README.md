...
- 长视频支持“全文投喂AI”：页面不默认展开长篇全文，但可一键复制完整字幕/转录给 AI。
...
- X：已验证可用；公开内容可尝试直接提取，登录可见内容需要 Cookie。X 帖子可能同时带缩略图和长视频，入库前会先检查 `duration`、`formats`、`subtitles` 等元数据；只要检测到视频，就走视频下载/转录流程，不按纯图文处理。
...
导出脚本默认会原样保留 `transcript.txt`，适合已经做过 AI 语义校对或人工整理的稿子。只有需要把原始单行稿重新自动断句时，才额外加 `--format-transcript`。长视频默认在笔记中展示 AI 语义整理稿，并挂上完整纯文本字幕、原始 VTT/原始分段稿链接；只有明确要求全文展示时，才把逐字完整版直接写进笔记正文。
